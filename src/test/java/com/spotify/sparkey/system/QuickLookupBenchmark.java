...
public class QuickLookupBenchmark {
  ...
  public void setup() throws IOException {
    ...
    bitmask = numElements - 1;

    SparkeyWriter writer = Sparkey.createNew(indexFile, compressionType, 1024);
    final List<String> keys = new ArrayList<>();
    for (int i = 0; i < numElements; i++) {
      final String key = "key_" + i;
      writer.put(key, "value_" + i);
      keys.add(key);
    }
    Collections.shuffle(keys, new Random(891273791623L));
    keysArray = keys.toArray(new String[0]);
    writer.writeHash();
    writer.close();
  ...
  public String test() throws IOException {
    return reader.getAsString("key_" + keysArray[++counter & bitmask]);
  }
}
